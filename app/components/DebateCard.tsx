...
export function DebateCard({
  ...
  return (
    <div className="border border-glass bg-[#0F0F1A] rounded-lg overflow-hidden hover:bg-[#121220] transition-colors h-full flex flex-col">
      {/* Header */}
      <div className="border-b border-glass px-4 py-3 flex items-center justify-between bg-dark-overlay/50">
        <div className="flex items-center gap-2">
          <span className="text-lg">{personaEmoji}</span>
          <div>
            <div className="text-sm font-medium text-white">{modelName}</div>
            <div className="text-xs text-gray-500 mono-meta">{modelId}</div>
          </div>
        </div>
        {!isComplete && (
      ...
      <div className="flex-1 overflow-y-auto p-4">
        <div className="text-sm leading-relaxed text-gray-300 whitespace-pre-wrap break-words">
          {content || (
